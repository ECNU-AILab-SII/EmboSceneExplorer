...
def load_pretrained_model(model_path, model_base, model_name, pointcloud_tower_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if os.path.exists(os.path.join(model_path, 'non_lora_trainables.bin')):
                print('Using local pretrained checkpoints.')
            ...
            # ========== 这里插入扩展 embedding 层的代码 ==========
            tokenizer_vocab_size = len(tokenizer)
            model_vocab_size = model.lm_head.out_features  # 或 model.get_input_embeddings().num_embeddings

            if tokenizer_vocab_size > model_vocab_size:
                print(f"扩展 embedding 层: {model_vocab_size} -> {tokenizer_vocab_size}")
                # 扩展 embedding 层
                old_embed = model.get_input_embeddings().weight.data
                new_embed = torch.cat([
                    old_embed,
                    torch.zeros(tokenizer_vocab_size - model_vocab_size, old_embed.shape[1], device=old_embed.device, dtype=old_embed.dtype)
                ], dim=0)
                model.get_input_embeddings().weight = torch.nn.Parameter(new_embed)

                # 扩展 lm_head 层
                old_lm_head = model.lm_head.weight.data
                new_lm_head = torch.cat([
                    old_lm_head,
                    torch.zeros(tokenizer_vocab_size - model_vocab_size, old_lm_head.shape[1], device=old_lm_head.device, dtype=old_lm_head.dtype)
                ], dim=0)
                model.lm_head.weight = torch.nn.Parameter(new_lm_head)
            # ========== 这里结束 ==========
    ...
    # 在模型和tokenizer都加载后，检查并扩展embedding
    tokenizer_vocab_size = len(tokenizer)
    model_vocab_size = model.lm_head.out_features  # 或 model.get_input_embeddings().num_embeddings
