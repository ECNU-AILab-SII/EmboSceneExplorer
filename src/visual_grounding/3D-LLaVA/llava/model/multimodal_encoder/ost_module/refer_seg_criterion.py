    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores,
            ...
            if idx_q >= 0 and idx_gt >=0:
                ...
                pred_mask.float(), tgt_mask.float(),reduction='none')
                loss_per_flat = loss_per.detach().cpu().numpy().reshape(-1, 1) # Reshape to (num_elements, 1)
                gmm_I = GaussianMixture(n_components=2, max_iter=100, tol=1e-2, reg_covar=5e-4)
                gmm_I.fit(loss_per_flat)
                prob_I = gmm_I.predict_proba(loss_per_flat)
                prob_np = prob_I[:, gmm_I.means_.argmin()]
                prob_tensor=torch.from_numpy(prob_np).to(loss_per.device)
                mask_bce_losses.append((prob_tensor*loss_per).mean())
                #######GMM#################
