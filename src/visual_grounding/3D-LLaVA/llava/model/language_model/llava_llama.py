...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward_mask_decoder_train(
        ...
        # zero_sp_features = [torch.zeros_like(t) for t in sp_features]
        # zero_pc_hidden_states = [[torch.zeros_like(t) for t in sublist] for sublist in pc_hidden_states]
        ...
        total_batch_loss = 0
        cl_loss=0
        num_valid_batch_items=0
        margin=1.0
        num_pairs_per_sample=256
        device=sp_features[0].device
        if set(conditions) == {'refer_seg'}:
            for i in range(BS):
                try:
                    features_single = sp_features[i] # 当前样本的特征 (N, D)
                    mask_single = gt_seg_masks[i]    # 当前样本的掩码 (N,)
                    positive_indices = torch.where(gt_seg_masks[i] == 1)[1]
                    negative_indices = torch.where(gt_seg_masks[i] == 0)[1]
                    # 检查是否有足够的点来形成正负对
                    # 至少需要2个正样本点来形成正对，至少1个负样本点来形成负对
                    if len(positive_indices) < 2 or len(negative_indices) < 1:
                        continue # 如果条件不足，跳过这个样本

                    # --- 采样正样本对 ---
                    # 从正样本中随机选出 anchor 和 positive
                    num_pos_pairs = min(num_pairs_per_sample // 2, len(positive_indices) // 2)
                    if num_pos_pairs == 0: continue

                    # 随机选择两组索引，用于构建正样本对
                    idx1_pos = positive_indices[torch.randint(0, len(positive_indices), (num_pos_pairs,), device=device)]
                    idx2_pos = positive_indices[torch.randint(0, len(positive_indices), (num_pos_pairs,), device=device)]
                    
                    # 确保不选择同一个点作为正样本对（可选，但在大N下概率低）
                    # while (idx1_pos == idx2_pos).any(): # 如果需要严格不重复，可以加这个循环，但可能效率低
                    #     idx2_pos = positive_indices[torch.randint(0, len(positive_indices), (num_pos_pairs,), device=device)]
                        
                    feat1_pos = features_single[idx1_pos]
                    feat2_pos = features_single[idx2_pos]
                    # --- 你原来的代码 ---
                    normal1_pos = normal[i][idx1_pos]
                    normal2_pos = normal[i][idx2_pos]
                    dot_product = torch.sum(normal1_pos * normal2_pos, axis=1)
                    norm1 = torch.linalg.norm(normal1_pos, axis=1)
                    norm2 = torch.linalg.norm(normal2_pos, axis=1)
                    # 为防止除以零，增加一个很小的数 epsilon
                    cosine_similarity = dot_product / (norm1 * norm2 + 1e-8)

                    # --- 新增的转换代码 ---
                    # 将余弦相似度 [-1, 1] 转换为相异度 [0, 1]
                    # 越相似，值越小
                    dissimilarity = (1.0 - cosine_similarity) / 2.0
                    pos_dist_sq = F.pairwise_distance(feat1_pos, feat2_pos, p=2).pow(2)*dissimilarity
                    loss_pos = torch.mean(pos_dist_sq)

                    num_neg_pairs = min(num_pairs_per_sample // 2, len(positive_indices), len(negative_indices))
                    if num_neg_pairs == 0: continue
                    idx1_neg = positive_indices[torch.randint(0, len(positive_indices), (num_neg_pairs,), device=device)] # 锚点来自正样本
                    idx2_neg = negative_indices[torch.randint(0, len(negative_indices), (num_neg_pairs,), device=device)] # 负样本来自负样本
                    feat1_neg = features_single[idx1_neg]
                    feat2_neg = features_single[idx2_neg]
                    neg_dist_sq = F.pairwise_distance(feat1_neg, feat2_neg, p=2).pow(2)
                    mask = neg_dist_sq < 0.5  # 只优化距离小于阈值的负样本对
                    filtered_neg_dist = neg_dist_sq[mask]

                    if filtered_neg_dist.numel() > 0:
                        loss_neg = torch.mean(F.relu(margin - torch.sqrt(filtered_neg_dist)).pow(2))
                    else:
                        loss_neg = torch.tensor(0.0, device=feat1_neg.device)
                    # loss_neg = torch.mean(F.relu(margin - torch.sqrt(neg_dist_sq)).pow(2))
                    
                    total_batch_loss += (loss_pos + loss_neg)
                    num_valid_batch_items += 1
                except:
                    print("error")
                    print(gt_seg_masks[i])
                # 如果有有效样本，返回平均损失；否则返回0
                if num_valid_batch_items > 0:
                    cl_loss=total_batch_loss / num_valid_batch_items
                else:
                    cl_loss=torch.tensor(0.0, device=device)
                    print(cl_loss)
                llm_out["loss"] += cl_loss
